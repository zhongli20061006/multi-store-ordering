...
- 测试：43 passed（鉴权、门店归属、菜单、订单金额、状态机、幂等、限库存、取消分类与回补、查单收紧、脱敏、索引、越权）
- 实机验证：/api/v1/health 返回 ok；下单扣库存（5→4）、顾客取消后回补（→5）、商家列表脱敏（139****0001）、详情完整；跨店访问 403（自动化用例覆盖）
- 演示账号：admin1 → 中山路店，admin2 → 万达店，密码 admin123456（仅本地开发用，正式使用前必须改）
- 2026-08-04：表结构变更（order_status 更名、cancel_reason/cancel_by），本地演示库已重建（仅种子数据）
