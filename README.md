...
- 门店主题换肤（四套：暖橙经典/纯白简约/深夜墨绿/莓果甜心，后台可切换）
...
| 账号 | 密码 | 绑定门店 |
| --- | --- | --- |
| admin1 | admin123456 | 中山路店（熟食/warm）、夜宵摊（夜宵/night） |
| admin2 | admin123456 | 万达店（奶茶/white）、甜品店（甜品/berry） |

演示店铺共四家，对应四种主题与餐饮模式（熟食/奶茶/夜宵/甜品），菜单已配齐；商品图留空待补。
...
- 阶段二（已完成 2.1-2.7）：小程序功能搭建与体验、商家后台随做
- 阶段三（执行中）：UI 重构——A 批（token + 高曝光页）已完成，B 批待走查
- 阶段四/五（挂起）：部署前准备（支付/登录/核销/部署物料，待资质）与正式上线
